...
export const syncKeywords = async (keywords: string[]) => {
  ...
  await Keywords.bulkWrite(operations)
}
...
export const syncTags = async (tags: string[]): Promise<string[]> => {
  ...
  for (const tagInput of tags) {
    ...
    if (tagInput.match(/^[0-9a-fA-F]{24}$/)) {
      const exists = await Keywords.findOne({ _id: tagInput, type: 'tag' })
      if (exists) {
    ...
    let tag = await Keywords.findOne({ $or: [{ name: tagInput }, { slug: slug }], type: 'tag' })

    if (!tag) {
      try {
        tag = await Keywords.create({ name: tagInput, type: 'tag' })
      } catch (e) {
        tag = await Keywords.findOne({ name: tagInput, type: 'tag' })
      }
    }
