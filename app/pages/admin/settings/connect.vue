...
const { data: connects, refresh, pending } = await useAPI<Models.Response<Record<string, Models.Connect>>>('/api/settings/connect')
...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  try {
    ...
    if (isEdit.value && selectedId.value) {
      await $api(`/api/settings/connect/${selectedId.value}`, {
        method: 'PUT',
        body: payload
      })
    } else {
      ...
      await $api('/api/settings/connect', {
        method: 'POST',
        body: {
          [event.data.key]: {
            ...payload,
...
const onDelete = async (id?: string) => {
  ...
  try {
    await $api(`/api/settings/connect/${id}`, {
      method: 'DELETE'
    })
    toast.add({ title: $t('message.success'), color: 'success' })
    refresh()
...
const onToggleStatus = async (row: Models.Connect) => {
  try {
    ...
    await $api('/api/settings/connect/status', {
      method: 'PUT',
      body: {
        id: row._id,
        flag: newFlag
