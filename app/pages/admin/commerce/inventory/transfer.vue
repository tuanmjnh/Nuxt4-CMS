...
const schema = z.object({
  ...
  toWarehouse: z.string().min(1, $t('form.required')),
...
}).refine(data => data.warehouse !== data.toWarehouse, {
...
<template>
  <div class="p-4">
    <div class="flex items-center gap-4 mb-4">
      <UButton icon="i-lucide-arrow-left" color="neutral" variant="ghost" to="/admin/products/inventory" />
      <h1 class="text-2xl font-bold">{{ $t('inventory.transfer') }}</h1>
    </div>
    ...
    <UCard>
      <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
        <div class="grid grid-cols-2 gap-4">
          ...
          <UFormGroup :label="$t('inventory.to_warehouse')" name="toWarehouse" required>
            <USelectMenu v-model="state.toWarehouse" :options="warehouses || []" value-attribute="_id"
              option-attribute="name" searchable />
          </UFormGroup>
        </div>
        ...
        <div class="border rounded p-4 space-y-4">
          <div class="flex justify-between items-center">
            ...
            <UButton size="xs" icon="i-lucide-plus" @click="addItem">
              {{ $t('common.add') }}
            </UButton>
          </div>
          ...
          <div v-for="(item, index) in state.items" :key="index"
            ...
            <UButton color="error" variant="ghost" icon="i-lucide-trash" @click="removeItem(index)" />
          </div>
        </div>
