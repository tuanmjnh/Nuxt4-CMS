...
const { data: routesData, refresh } = await useAPI<any>('/api/system/routes')
const formRef = ref()
...
const saveOrder = async (tree: any[]) => {
  ...
  try {
    await $fetch('/api/system/routes/order', {
      method: 'PUT',
...
const toggleVisibility = async (row: any) => {
  ...
  try {
    await $fetch(`/api/system/routes/${row._id}`, {
      method: 'PUT',
      headers: { Authorization: `Bearer ${token.value}` },
...
const onSubmit = async (event: FormSubmitEvent<Schema>) => {
  try {
    if (isEditing.value) {
      await $fetch(`/api/system/routes/${editingId.value}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token.value}` },
        body: event.data
      })
